...
public class SystemConfiguration{
    ...
    public static Boolean getDisableAutoconfigure(){
        try {
            Context env = (Context)new InitialContext().lookup("java:comp/env");
            return (Boolean)env.lookup(Constants.INIT_OIOSAML_DISABLEAUTOCONFIGURE);
        } catch (NamingException e) {
            return null;
        }
    }
    ...
    public static String getConfigurationClass(){
        try {
            Context env = (Context)new InitialContext().lookup("java:comp/env");
            return (String)env.lookup(Constants.INIT_OIOSAML_CONFIGURATION_CLASS);
        } catch (NamingException e) {
            return null;
        }
    }
    ...
    public static String getDatasourceJNDI(){
        try {
            Context env = (Context)new InitialContext().lookup("java:comp/env");
            return (String)env.lookup(Constants.INIT_OIOSAML_DATASOURCE_JNDI);
        } catch (NamingException e) {
            return null;
        }
    }
